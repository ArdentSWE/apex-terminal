...
const GlobalDashboard = React.memo(function GlobalDashboard() {
  ...
  // Accept the new strict AI strings
  const dayTrades = plays.filter((p: any) => 
    p.play_type === "DAY TRADE" || 
    p.play_type === "0DTE SCALP" || 
    p.play_type === "WEEKLY SWING"
  );
  
  const swings = plays.filter((p: any) => 
    p.play_type === "SWING"
  );
  
  const leaps = plays.filter((p: any) => 
    p.play_type === "LEAP" || 
    p.play_type === "WHALE"
  );
